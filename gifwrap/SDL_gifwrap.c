...
static Uint8 gif_find_matching_index(SDL_Palette* palette, SDL_Color background_color) {
    ...
    int i;
    for (i = 0; i < num_colors; ++i) {
        if (background_color.r == palette->colors[i].r && background_color.g == palette->colors[i].g && background_color.b == palette->colors[i].b)
            return i;
    }
...
// Sets the number of animation loops.  0 loops means to looped forever.
